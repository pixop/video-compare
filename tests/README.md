...
- `buffer-forward-only`
- `buffer-pingpong`
- `crop-copy` — Shift+L/R mouse crop, Shift+O/I, Backspace undo, and right-video selection through the real Display → VideoCompare → VideoFilterer path. Asserts `crop=` in the Shift+X filter dump, plus `swapped` / `right` / visual-size ownership across Swap. Uses 320×180 / 640×360 / 160×90 rights so copy is normalized, not pixel-identical. Spatial post-filter crop-space is covered in the Video filterer unit tests rather than here: a post-scale on one clip would change displayed size, canvas, and interactive-crop mapping while leaving crop-space unchanged, and would require rewriting the existing dump indexes.
