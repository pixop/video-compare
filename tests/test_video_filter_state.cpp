...
int main() {
  ...
  expect_bool("seed Right 1 before Ctrl+Shift+R", video_filter_state::apply_crop_to_indices(identity, identity_ops, right1_only, 1, right_a), true);
  expect_bool("Ctrl+Shift+R from Right 1 to left", video_filter_state::copy_crop(identity, identity_ops, CropCopyRequest::ActiveRightToLeft, false, kLeft, kRight0, 3, 1), true);
  expect_operation("Ctrl+Shift+R records left only", identity_ops.back(), {kLeft});
  expect_bool("undo Ctrl+Shift+R after selecting Right 2", video_filter_state::undo_last_crop_operation(identity, identity_ops), true);
  expect_crop("Ctrl+Shift+R undo restores left", identity[kLeft].crop, no_crop);
  expect_crop("Ctrl+Shift+R undo leaves Right 1 source crop", identity[kRight1].crop, right_a);
  expect_crop("Ctrl+Shift+R undo leaves Right 2 untouched", identity[kRight2].crop, no_crop);
  ...
  expect_bool("Ctrl+Shift+L when every right already matches left is a no-op", video_filter_state::copy_crop(nops, nop_ops, CropCopyRequest::LeftToAllRights, false, kLeft, kRight0, 3, 0), false);
  expect_size("identical Ctrl+Shift+L does not consume an undo step", nop_ops.size(), 2);
  expect_bool("Ctrl+Shift+R when left already matches current right is a no-op", video_filter_state::copy_crop(nops, nop_ops, CropCopyRequest::ActiveRightToLeft, false, kLeft, kRight0, 3, 0), false);
  expect_size("identical Ctrl+Shift+R does not consume an undo step", nop_ops.size(), 2);
  ...
  expect_bool("partial Ctrl+Shift+L changes only mismatched rights", video_filter_state::copy_crop(partial, partial_ops, CropCopyRequest::LeftToAllRights, false, kLeft, kRight0, 3, 0), true);
  expect_operation("partial Ctrl+Shift+L records only Right 1 and Right 2", partial_ops.back(), {kRight1, kRight2});
  expect_crop("partial Ctrl+Shift+L leaves already-matching Right 0", partial[kRight0].crop, left_crop);
  expect_bool("undo partial Ctrl+Shift+L", video_filter_state::undo_last_crop_operation(partial, partial_ops), true);
  ...
  expect_bool("Ctrl+Shift+L maps left crop to each right's size", video_filter_state::copy_crop(sized, sized_ops, CropCopyRequest::LeftToAllRights, false, kLeft, kRight0, 3, 0), true);
  expect_crop("Ctrl+Shift+L 3840 dest is 2x", sized[kRight0].crop, CropState{{200, 100, 3200, 1800}, true});
  expect_crop("Ctrl+Shift+L same-size dest is identical", sized[kRight1].crop, hd_crop);
  expect_crop("Ctrl+Shift+L 1280 dest is edge-mapped", sized[kRight2].crop, mapped_720);
  expect_crop("Ctrl+Shift+L leaves source left", sized[kLeft].crop, hd_crop);
  expect_bool("undo sized Ctrl+Shift+L", video_filter_state::undo_last_crop_operation(sized, sized_ops), true);
  expect_crop("undo sized Ctrl+Shift+L restores Right 0 pixels", sized[kRight0].crop, right_a);
  expect_crop("undo sized Ctrl+Shift+L restores Right 1 pixels", sized[kRight1].crop, right_b);
  expect_crop("undo sized Ctrl+Shift+L restores Right 2 none", sized[kRight2].crop, no_crop);
  ...
  expect_bool("swapped Ctrl+Shift+L maps selected right onto visual right only", video_filter_state::copy_crop(sized, sized_ops, CropCopyRequest::LeftToAllRights, true, kLeft, kRight0, 3, 1), true);
  expect_crop("swapped Ctrl+Shift+L visual right received source crop", sized[kLeft].crop, hd_crop);
  expect_crop("swapped Ctrl+Shift+L does not copy onto other rights", sized[kRight0].crop, sized_right0_before_swap_o);
  expect_size("swapped Ctrl+Shift+L does not push history on other rights", sized[kRight0].history.size(), sized_right0_history_before_swap_o);
  expect_crop("swapped Ctrl+Shift+L source Right 1 unchanged", sized[kRight1].crop, hd_crop);
  expect_crop("swapped Ctrl+Shift+L leaves unselected Right 2", sized[kRight2].crop, no_crop);
  expect_bool("undo swapped sized Ctrl+Shift+L", video_filter_state::undo_last_crop_operation(sized, sized_ops), true);
  expect_crop("undo swapped sized Ctrl+Shift+L restores visual right", sized[kLeft].crop, right_b);
  expect_crop("undo swapped sized Ctrl+Shift+L leaves Right 0", sized[kRight0].crop, sized_right0_before_swap_o);
  expect_crop("undo swapped sized Ctrl+Shift+L leaves Right 1", sized[kRight1].crop, hd_crop);
  ...
  sized[kRight1].crop = right_b;
  sized[kRight1].history = {right_b};
  expect_bool("Ctrl+Shift+R maps current right onto left", video_filter_state::copy_crop(sized, sized_ops, CropCopyRequest::ActiveRightToLeft, false, kLeft, kRight0, 3, 1), true);
  expect_crop("Ctrl+Shift+R same-size left received Right 1", sized[kLeft].crop, right_b);
  expect_bool("undo sized Ctrl+Shift+R", video_filter_state::undo_last_crop_operation(sized, sized_ops), true);
  expect_crop("undo sized Ctrl+Shift+R restores left pixels", sized[kLeft].crop, hd_crop);
  expect_bool("swapped Ctrl+Shift+R maps left onto 3840 right", video_filter_state::copy_crop(sized, sized_ops, CropCopyRequest::ActiveRightToLeft, true, kLeft, kRight0, 3, 0), true);
  expect_crop("swapped Ctrl+Shift+R Right 0 received 2x left", sized[kRight0].crop, CropState{{200, 100, 3200, 1800}, true});
  expect_crop("swapped Ctrl+Shift+R leaves left", sized[kLeft].crop, hd_crop);
  expect_bool("undo swapped sized Ctrl+Shift+R", video_filter_state::undo_last_crop_operation(sized, sized_ops), true);
  expect_crop("undo swapped sized Ctrl+Shift+R restores Right 0 pixels", sized[kRight0].crop, right_a);
  ...
  expect_bool("Ctrl+Shift+L normal source is logical left", o_normal.source_is_left, true);
  expect_bool("Ctrl+Shift+L normal destinations are all rights", o_normal.copy_to_all_rights, true);
  const auto o_swap = video_filter_state::resolve_crop_copy(CropCopyRequest::LeftToAllRights, true, 1, 3);
  expect_bool("Ctrl+Shift+L swapped source is current right", o_swap.source_is_left, false);
  expect_size("Ctrl+Shift+L swapped source is snapshotted Right 1", o_swap.source_right_index, 1);
  expect_bool("Ctrl+Shift+L swapped destinations are not all rights", o_swap.copy_to_all_rights, false);
  expect_bool("Ctrl+Shift+L swapped copies only onto visual right (logical left)", o_swap.dest_is_left, true);
  expect_bool("Ctrl+Shift+L normal does not copy onto logical left", o_normal.dest_is_left, false);
  const auto i_normal = video_filter_state::resolve_crop_copy(CropCopyRequest::ActiveRightToLeft, false, 1, 3);
  expect_bool("Ctrl+Shift+R normal source is current right", i_normal.source_is_left, false);
  expect_size("Ctrl+Shift+R normal source is snapshotted Right 1", i_normal.source_right_index, 1);
  expect_bool("Ctrl+Shift+R normal dest is logical left", i_normal.dest_is_left, true);
  const auto i_swap = video_filter_state::resolve_crop_copy(CropCopyRequest::ActiveRightToLeft, true, 1, 3);
  expect_bool("Ctrl+Shift+R swapped source is logical left", i_swap.source_is_left, true);
  expect_bool("Ctrl+Shift+R swapped dest is current right", i_swap.dest_is_left, false);
  expect_size("Ctrl+Shift+R swapped dest is snapshotted Right 1", i_swap.dest_right_index, 1);
  ...
  expect_bool("deferred Ctrl+Shift+L keeps snapshotted Swap source as current right", deferred_o_plan.source_is_left, false);
  expect_size("deferred Ctrl+Shift+L keeps snapshotted Right 1", deferred_o_plan.source_right_index, 1);
  expect_bool("deferred Ctrl+Shift+L keeps snapshotted visual-right dest", deferred_o_plan.dest_is_left, true);
  expect_bool("deferred Ctrl+Shift+L does not copy onto other rights", deferred_o_plan.copy_to_all_rights, false);
  ...
  expect_bool("deferred Ctrl+Shift+R keeps snapshotted Swap source as logical left", deferred_i_plan.source_is_left, true);
  expect_bool("deferred Ctrl+Shift+R keeps snapshotted dest as Right 1", deferred_i_plan.dest_is_left, false);
  expect_size("deferred Ctrl+Shift+R dest stays snapshotted Right 1", deferred_i_plan.dest_right_index, 1);
  ...
  expect_bool("Ctrl+Shift+L normal copies logical left to all rights", video_filter_state::copy_crop(visual, visual_ops, CropCopyRequest::LeftToAllRights, false, kLeft, kRight0, 3, 0), true);
  expect_operation("Ctrl+Shift+L normal records all rights", visual_ops.back(), {kRight0, kRight1, kRight2});
  expect_crop("Ctrl+Shift+L normal Right 0 received left", visual[kRight0].crop, left_crop);
  expect_crop("Ctrl+Shift+L normal Right 1 received left", visual[kRight1].crop, left_crop);
  expect_crop("Ctrl+Shift+L normal Right 2 received left", visual[kRight2].crop, left_crop);
  expect_crop("Ctrl+Shift+L normal leaves logical left unchanged", visual[kLeft].crop, left_crop);
  expect_bool("undo Ctrl+Shift+L normal after later swap/selection", video_filter_state::undo_last_crop_operation(visual, visual_ops), true);
  expect_crop("Ctrl+Shift+L normal undo restores Right 0", visual[kRight0].crop, right_a);
  expect_crop("Ctrl+Shift+L normal undo restores Right 1", visual[kRight1].crop, right_b);
  expect_crop("Ctrl+Shift+L normal undo restores Right 2", visual[kRight2].crop, no_crop);
  expect_crop("Ctrl+Shift+L normal undo leaves left", visual[kLeft].crop, left_crop);

  expect_bool("Ctrl+Shift+L swapped copies visual left onto visual right only", video_filter_state::copy_crop(visual, visual_ops, CropCopyRequest::LeftToAllRights, true, kLeft, kRight0, 3, 1), true);
  expect_operation("Ctrl+Shift+L swapped records visual right only", visual_ops.back(), {kLeft});
  expect_crop("Ctrl+Shift+L swapped visual right (logical left) received source crop", visual[kLeft].crop, right_b);
  expect_crop("Ctrl+Shift+L swapped leaves other Right 0", visual[kRight0].crop, right_a);
  expect_crop("Ctrl+Shift+L swapped source Right 1 unchanged", visual[kRight1].crop, right_b);
  expect_crop("Ctrl+Shift+L swapped leaves other Right 2", visual[kRight2].crop, no_crop);
  expect_size("Ctrl+Shift+L swapped records visual right (logical left)", visual[kLeft].history.size(), left_history_before_swap_o + 1);
  expect_size("Ctrl+Shift+L swapped does not push history on other Right 0", visual[kRight0].history.size(), right0_history_before_swap_o);
  expect_size("Ctrl+Shift+L swapped does not push history on source right", visual[kRight1].history.size(), right1_history_before_swap_o);
  expect_size("Ctrl+Shift+L swapped does not push history on other Right 2", visual[kRight2].history.size(), right2_history_before_swap_o);
  expect_bool("undo Ctrl+Shift+L swapped after later swap/selection", video_filter_state::undo_last_crop_operation(visual, visual_ops), true);
  expect_crop("Ctrl+Shift+L swapped undo restores left", visual[kLeft].crop, left_crop);
  expect_crop("Ctrl+Shift+L swapped undo leaves Right 0", visual[kRight0].crop, right_a);
  expect_crop("Ctrl+Shift+L swapped undo leaves source Right 1", visual[kRight1].crop, right_b);
  expect_crop("Ctrl+Shift+L swapped undo leaves Right 2", visual[kRight2].crop, no_crop);

  expect_bool("Ctrl+Shift+R normal copies current right to left", video_filter_state::copy_crop(visual, visual_ops, CropCopyRequest::ActiveRightToLeft, false, kLeft, kRight0, 3, 0), true);
  expect_operation("Ctrl+Shift+R normal records left only", visual_ops.back(), {kLeft});
  expect_crop("Ctrl+Shift+R normal left received Right 0", visual[kLeft].crop, right_a);
  expect_crop("Ctrl+Shift+R normal leaves Right 0", visual[kRight0].crop, right_a);
  expect_crop("Ctrl+Shift+R normal leaves Right 1", visual[kRight1].crop, right_b);
  expect_bool("undo Ctrl+Shift+R normal after later swap/selection", video_filter_state::undo_last_crop_operation(visual, visual_ops), true);
  expect_crop("Ctrl+Shift+R normal undo restores left", visual[kLeft].crop, left_crop);
  expect_crop("Ctrl+Shift+R normal undo leaves Right 0", visual[kRight0].crop, right_a);
  expect_crop("Ctrl+Shift+R normal undo leaves Right 1", visual[kRight1].crop, right_b);

  expect_bool("Ctrl+Shift+R swapped copies left to current right", video_filter_state::copy_crop(visual, visual_ops, CropCopyRequest::ActiveRightToLeft, true, kLeft, kRight0, 3, 1), true);
  expect_operation("Ctrl+Shift+R swapped records snapshotted Right 1", visual_ops.back(), {kRight1});
  expect_crop("Ctrl+Shift+R swapped Right 1 received left", visual[kRight1].crop, left_crop);
  expect_crop("Ctrl+Shift+R swapped leaves logical left", visual[kLeft].crop, left_crop);
  expect_crop("Ctrl+Shift+R swapped leaves Right 0", visual[kRight0].crop, right_a);
  expect_bool("undo Ctrl+Shift+R swapped after later swap/selection", video_filter_state::undo_last_crop_operation(visual, visual_ops), true);
  expect_crop("Ctrl+Shift+R swapped undo restores Right 1", visual[kRight1].crop, right_b);
  expect_crop("Ctrl+Shift+R swapped undo leaves left", visual[kLeft].crop, left_crop);
  expect_crop("Ctrl+Shift+R swapped undo leaves Right 0", visual[kRight0].crop, right_a);
  expect_crop("Ctrl+Shift+R swapped undo leaves Right 2", visual[kRight2].crop, no_crop);
