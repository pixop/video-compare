...
# Function to download and extract the latest SDL library
# Parameters:
...
download_sdl_library() {
    ...
    echo "Fetching the latest $REPO_NAME release data from GitHub API..."
    ...
    if [ -z "$WIN32_X64_URL" ]; then
        echo "Failed to find the latest $FILE_NAME win32-x64 build."
        return 1
    fi
    if [ -z "$MINGW_URL" ]; then
        echo "Failed to find the latest $FILE_NAME-devel mingw build."
        return 1
    fi
